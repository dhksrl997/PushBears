...
public class SingleCanvas extends Canvas { // 싱글플레이 vs Ai
	...
	public SingleCanvas() {
		...
		items = new Movable[100]; // Movable 객체 배열 100개 선언
		...
		try { // 카운트 다운 및 게임오버 화면
			...
			gameOverImg = ImageIO.read(new File("res/images/GameOver2.png"));//게임 오버 이미지
		...
		items[unitIndex++] = backButton; //뒤로가기 버튼
	...
	} // end public SingleCanvas() 

	public void randompush() { // Ai 작동
	...
	public void moveUpdate() {// 단위 이동후업데이트
		...
		if (--snowCnt == 0) { // 눈덩이카운트가 0이면 눈덩이 투척

			items[unitIndex++] = olaf.throwSnow(); // 올라프가 눈덩이 투척
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < unitIndex; i++) { // 게임 시간 구현
			items[i].draw(gg);
		}
		...
		if ((backTime == 0 && frontTime == 0) || Character.isBearDrop()) { // 시간 초 끝나면 GameOver이미지 출력
			gg.drawImage(gameOverImg, 400, 100, SingleCanvas.getInstacne()); // 카운트 다운 00 되면 gameover화면 나옴
			isStop=false;
			backTime = 0;
			frontTime = 0;
		}
